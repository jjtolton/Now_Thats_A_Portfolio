...
passphrases = """oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
def valid(passphrase):
    words = sorted(map(lambda x: tuple(sorted(x)), passphrase.split()))
    return sorted(set(words)) == words


def tests():
    docstring = """abcde fghij is a valid passphrase.
    ...
    for line in docstring.splitlines():
        p = '.*(?=\sis)'
        np = 'is not'
        passphrase = re.findall(p, line)[0].strip()
        yea = not re.findall(np, line)
        nay = not yea

        if yea:
            assert valid(passphrase)
        if nay:
            assert not valid(passphrase)


def main():
    s = 0
    for phrase in passphrases.strip().splitlines():
        if valid(phrase.strip()):
            s += 1
    print(s)
    return s
