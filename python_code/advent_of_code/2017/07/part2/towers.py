...
class Tower:
    def __init__(self, parent=None, children=None, weight=None, name=None):
        self.parent = parent
...
p = re.compile('(.*?(?=\s))(?:\s*)(?:\()(\d*)(?:\))(?:\s*->\s*)(.*) |'
               '(.*?(?=\s))(?:\s*)(?:\()(\d*)(?:\))',
               flags=re.VERBOSE)


def gather_towers(towers):
    res = []
    for tower in towers:
        r = filterv(None, re.match(p, tower).groups())
        if len(r) == 3:
            name, weight, children = r
            weight = int(weight)
            children = children.split(', ')
            t = Tower(name=name, weight=weight, children=children)
            res.append(t)
        if len(r) == 2:
            name, weight = r
            weight = int(weight)
...
def assign_parents(towers):
    for tower in towers:
        for t in towers:
            if t.children is not None and tower.name in t.children:
                tower.parent = t.name
    return towers


def assign_children(towers):
    for tower in towers:
        children = []
        for t in towers:
            if tower.children and t.name in tower.children:
                children.append(t)
...
def wrong_weight(b):
    m = tower_stats(b)
    mcw = defaultdict(int)
    for w in map(weight, b.children):
        mcw[w] += 1
    mcw = max(mcw, key=lambda x: mcw[x])
    worst = wk, wv = max(((k, v) for k, v in m.items()), key=lambda x: abs(x[1]['w'] - mcw))
    best = bk, bv = min(((k, v) for k, v in m.items()), key=lambda x: abs(x[1]['w'] - mcw))
    if wv != bv:
        return worst
    return False
...
def adjustment(b):
    ...
    best = bk, bv = min(((k, v) for k, v in m.items()), key=lambda x: abs(x[1]['w'] - mcw))
    if wv['w'] > bv['w']:
        return wv['p'] - abs(wv['w'] - bv['w'])
    else:
        return wv['p'] + abs(wv['w'] - bv['w'])


def tower_stats(b):
    m = {n.name: {'w': weight(n), 'p': n.weight} for n in b.children}
    return m


def wrongest(towers, prev=None):
    b = bottom(towers)
    w = wrong_weight(b)
    while w:
        kids = b.children
        names = [getattr(n, 'name') for n in kids]
        idx = names.index(w[0])
        prev = b
        b = b.children[idx]
        w = wrong_weight(b)
...
def weight(t):
    ...
    return t.weight + sum(weight(c) for c in t.children)


def find_adjustment(towers):
    towers = gather_towers(towers.strip().splitlines())
    towers = assign_parents(towers)
    towers = assign_children(towers)
    b = wrongest(towers)
    return adjustment(b)


def test():
    assert find_adjustment(sample) == 60


def main():
    print(find_adjustment(towers))
