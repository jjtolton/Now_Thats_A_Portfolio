...
from naga import mapv, get, update, fpartial, last

registers = """sdj dec 558 if r <= 8
...
p = re.compile('\s*(.*?)\s*(inc|dec)\s*([-]\d*|\d*)\s*if\s(.*)')


def parse_instruction(l):
    rname, ins, n, pred = re.match(p, l).groups()
    return Instruction(rname, ins, int(n), pred)


def parse_instructions(ls):
    return mapv(parse_instruction, ls.splitlines())


def get_registers(instructions):
    return {getattr(k, 'name'): 0 for k in instructions}


class Instruction:
    __slots__ = 'name', 'instruction', 'n', 'pred'

    def __init__(self, name, instruction, n, pred):
        self.name = name
        self.instruction = {'inc': operator.add, 'dec': operator.sub}[instruction]
        self.n = n
        self.pred = pred

    def __call__(self, registers):
        p = '(.*?)\s*(<=|==|!=|>=|<|>)\s*(.*)'
        rn, op, v = re.match(p, self.pred).groups()
        ...
        if op(pr, v):
            return update(registers, self.name, fpartial(self.instruction, self.n))
        else:
            return registers

    def __repr__(self):
        return f'Instruction(name={self.name}, ins={self.instruction}, n={self.n}, pred={self.pred})'
...
def nregisters(rs):
    return len({get(r, 'name') for r in rs})


def run(rs):
    ins = parse_instructions(rs)
    regs = get_registers(ins)
    for i in ins:
        regs = i(regs)
        yield regs
...
def max_run(runs):
    return max(max(runs, key=lambda r: max(r.values())).values())



def tests():
    sample_registers = """b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10"""
    assert max(last(run(sample_registers)).values()) == 1


def main():
    assert max_run(run(registers)) == 5035
